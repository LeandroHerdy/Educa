...
class OwnerCourseEditMixin(OwnerCourseMixin, OwnerEditMixin):
    tamplate_name = 'course/manage/course/list.html'
...
class CourseUpdateView(OwnerCourseMixin, UpdateView):
    permission_reqired = 'courses.change_course'
...
class CoursemoduleUpdateView(TemplateResponseMixin, View):
    ...
    def get_formset(self, data=None):
        return ModuleFormSet(instance=self.couse, data=data)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            formset.save()
            return redirect('manage_couse_list')
        return self.render_to_response({'course': self.course, 'formset':formset})        
...
class ContentCreateUpdateView(TemplateResponseMixin, View):
    ...
    def dispatch(self, request, module_id, model_name, id):
    ...
    def post(self, request, module_id, module_name, id=None):
        form = self.get_form(self.module,
                            instance=self.obj,
                            data=request.POST,
                            files=request.FILES)
        if not id:
...
class CourseListView(TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        subject = Subject.objects.annotate(total_courses=Count('course'))
        courses = Course.objects.annotate(total_module=Count('modules'))
        if subject:
            subject = courses.filter(subject=subject)
